...
def _calc_future_steps(future_horizon_sec: float, step_sec: float) -> int:
    if step_sec <= 0:
        raise ValueError(f"step_sec must be > 0, got {step_sec}")
...
def create_nuscenes_infos(root_path,
    ...
    fut_ts = _calc_future_steps(future_horizon_sec, step_sec)
    ego_fut_ts = fut_ts
    ...
    if test:
        ...
        info_path = osp.join(out_path,
                             '{}_infos_test_12s.pkl'.format(info_prefix))
        ...
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(out_path,
                             '{}_infos_train_12s.pkl'.format(info_prefix))
        mmcv.dump(data, info_path)
        data['infos'] = val_nusc_infos
        info_val_path = osp.join(out_path,
                                 '{}_infos_val_12s.pkl'.format(info_prefix))
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if not test:
            ...
            assert ego_fut_trajs.shape[0] == ego_fut_ts
...
parser.add_argument(
    ...
    default=0.5,
    required=False,
    help='future step size in seconds')
parser.add_argument(
    '--out-dir',
    type=str,
    default='./data/kitti',
