...
class BEVRender:
    ...
    def _render_sdc_car(self):
        ...
        sdc_path = os.path.join(resource_dir, 'sdc_car.png')
        sdc_car_png = cv2.imread(sdc_path)
        if sdc_car_png is None:
            # Fallback: simple rectangle if asset is missing.
            car = patches.Rectangle((-1, -2), 2, 4, linewidth=1,
                                    edgecolor='black', facecolor='gray', zorder=2)
    ...
    def _render_legend(self):
        resource_dir = os.path.join(os.path.dirname(__file__), 'resources')
        legend_path = os.path.join(resource_dir, 'legend.png')
