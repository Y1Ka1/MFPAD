...
class Visualizer:
    def __init__(
        ...
        self.results = self._load_results(args.result_path)
    ...
    def _load_results(self, result_path):
        try:
            return mmcv.load(result_path)
        except RuntimeError as exc:
            if "Attempting to deserialize object on a CUDA device" in str(exc):
                return torch.load(result_path, map_location=torch.device("cpu"))
            raise
