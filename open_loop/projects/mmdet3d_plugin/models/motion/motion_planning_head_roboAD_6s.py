...
class MotionPlanningHeadroboAD_6s(BaseModule):
    def __init__(
        ...
        self.last_planning_classification = None
        self.last_planning_prediction = None
        self.last_final_planning_prediction = None
        self.last_plan_query = None
        self.last_ego_cmd = None
        nusc_root = os.environ.get("MOMAD_NUSC_ROOT", "data/nuscenes/")
        self.nusc = NuScenes(version='v1.0-trainval', dataroot=nusc_root, verbose=True)
    ...
    def _ensure_cache(self, batch_size, token_count, embed_dim, device):
        if (
            self.last_planning_classification is None
            or self.last_planning_classification.size(0) != batch_size
            or self.last_planning_classification.size(-1) != token_count
        ):
            self.last_planning_classification = torch.zeros(
                [batch_size, 1, token_count], device=device
            ).detach()
            self.last_planning_prediction = torch.zeros(
                [batch_size, 1, token_count, self.ego_fut_ts, 2], device=device
            ).detach()
            self.last_plan_query = torch.zeros(
                [batch_size, 1, token_count, embed_dim], device=device
            ).detach()
            self.last_final_planning_prediction = torch.zeros(
                [batch_size, self.ego_fut_ts, 2], device=device
            ).detach()
            self.last_ego_cmd = torch.zeros([batch_size, 3], device=device).detach()
    ...
    def select_currect_best_planning(self, last_final_planning_prediction, planning_prediction, current_ego_cmd, last_ego_cmd):
        ...
        #import pdb;pdb.set_trace()
        select_currect_best_planning_prediction = torch.where(
            ego_mask.view(-1, 1, 1),
            planning_prediction[0].squeeze(1)[torch.arange(bs), min_idx],
            torch.full((self.ego_fut_ts, 2), 999.9).repeat(bs, 1, 1).to(ego_mask.device),
        )
        #import pdb;pdb.set_trace()
    ...
    def forward(
        ...
        token_count = plan_query.size(2)
        embed_dim = plan_query.size(3)
        self._ensure_cache(bs, token_count, embed_dim, device)
        if prev_sample_token == "":
            self.last_planning_classification = torch.zeros(
                [bs, 1, token_count], device=device
            ).detach()
            self.last_planning_prediction = torch.zeros(
                [bs, 1, token_count, self.ego_fut_ts, 2], device=device
            ).detach()
            self.last_plan_query = torch.zeros(
                [bs, 1, token_count, embed_dim], device=device
            ).detach()
            self.last_final_planning_prediction = torch.zeros(
                [bs, self.ego_fut_ts, 2], device=device
            ).detach()
            self.last_ego_cmd = torch.zeros([bs, 3], device=device).detach()
        ...
        last_query_mask = torch.zeros(
            [select_last_idx.shape[0], 1, token_count, embed_dim], device=device
        )
