...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, seconds: int = 12) -> EvalBoxes:
    ...
    # Only keep samples from this split unless overridden for custom datasets.
    ...
    use_all_scenes = os.environ.get('MOMAD_USE_ALL_SCENES') == '1'
    for sample_token in sample_tokens_all:
        scene_token = nusc.get('sample', sample_token)['scene_token']
        scene_record = nusc.get('scene', scene_token)
        if use_all_scenes or scene_record['name'] in splits[eval_split]:
            sample_tokens.append(sample_token)
